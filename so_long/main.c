...
/*   Updated: 2023/02/22 10:58:43 by jcoquard         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	event_manager(int keycode, t_data *data)
{
	...
	if (keycode == 122 && data->entity->pos.y > 0)
	{
		printf("up\n");
		data->entity->pos.y -= 4;
	}
	...
	if (keycode == 113 && data->entity->pos.x > 0)
	{
		printf("left\n");
		data->entity->pos.x -= 4;
	}
	...
	mlx_put_image_to_window(data->win->mlx, data->win->win, data->renderer->img, 0, 0);
	return (keycode);
}
...
int	main(void)
{
	...
	draw_rectangle(&renderer, (t_rectangle){(t_point){0, 0}, 499, 499}, 0x00FF0000);
